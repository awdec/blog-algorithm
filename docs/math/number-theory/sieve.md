...
对于求和式：$S(n)=\sum\limits_{i=1}^n f(i)g(\lfloor\dfrac{n}{i}\rfloor)$，根据整除分块可得：$S(n)=\sum\limits_{i=1}^{m} g(\lfloor\dfrac{n}{l_i}\rfloor)\sum\limits_{j=l_i}^{r_i} f(j)$，其中 $m=O(\sqrt n)$，总时间复杂度：$O(\sqrt n T(n))$，其中 $T(n)$ 为计算 $f(n)$ 前缀和的时间复杂度。
